...
function useUser() {
  ...
  useEffect(() => {
    fetch('https://ag.goldencitycasino123.site/api/user', {
      headers: {
...
export default function DigitBetGame() {
  ...
  useEffect(() => {
    fetch('https://ag.goldencitycasino123.site/api/digitbet/history', {
      headers: {
        'Authorization': 'Bearer ' + localStorage.getItem('token'),
        'Accept': 'application/json'
      }
  ...
  const sendToBackend = async (bet, resultNumber, winStatus, digit = null) => {
    ...
    try {
      ...
      const response = await fetch('https://ag.goldencitycasino123.site/api/digitbet', {
        method: 'POST',
        headers: {
