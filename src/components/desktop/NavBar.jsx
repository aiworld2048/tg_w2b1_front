...
function NavBar() {
  ...
  return (
    <>
      ...
      <nav className={`w-full z-30 sticky pt-3 top-0 bg-[#071a53] shadow-lg transition-all duration-300 ${isScrolled ? 'shadow-2xl' : ''}`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between px-2 sm:px-6 py-2 gap-2">
          {/* Logo */}
          <div className="flex items-center gap-2 cursor-pointer flex-shrink-0" onClick={() => navigate('/')}> 
            <img src={'6tribet.jpg'} alt="Logo" className="h-10 md:h-10 w-30 object-contain" />
          </div>

          {/* Balances & Exchange - Always Centered */}
          {user && (
            <div className="flex items-center gap-1 bg-[#071a60] rounded-xl px-3 md:px-4 py-1 md:py-2 shadow-inner flex-1 justify-center mx-2 min-w-0 overflow-x-auto">
            ...
            {!user ? (
              <>
                <button 
                  ...
                  className="px-1 py-1 rounded-lg bg-[#23243a] text-white font-semibold hover:bg-yellow-400 hover:text-black transition text-xs md:text-base"
                >
                  {content?.auth?.login || "Login"}
                </button>
                {/* <button
                  onClick={() => setShowRegister(true)} 
                  className="px-1 py-1 rounded-lg bg-yellow-400 text-black font-semibold hover:bg-yellow-500 transition text-xs md:text-base"
                >
                  {content?.auth?.register || "Join Now"}
            </button> */}
               <a href="https://ag.6tribet.net/assets/6tribet.apk" download>
                <button   className="px-1 py-1 rounded-lg bg-[#23243a] text-white font-semibold hover:bg-yellow-400 hover:text-black transition text-xs md:text-base">
                   {content?.auth?.download || "Download"}
                </button>
              </a>
            </>
            ) : null}
